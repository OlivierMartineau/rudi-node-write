...
class RudiNodeJwtFactory(Connector):
    ...
    def test_connection(self):
        test = self.request(url='crypto/jwt', req_method='GET', headers=self._headers)
        if test['RUDI'] != 'JWT':
    ...
    def _renew_jwt(self, delay_s: int):
        ...
        'req_mtd': 'all',
        'req_url': 'all'}
        return self.request(url='crypto/jwt/forge', req_method='POST', body=jwt_body, headers=self._headers)
