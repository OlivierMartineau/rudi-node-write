from rudi_node_write.utils.type_string import check_is_string
from rudi_node_write.utils.types import is_type, is_list, is_list_or_dict, get_type_name
...
def check_has_key(obj: dict, key_name: str):
    if not has_key(check_is_dict(obj), check_is_string(key_name)):
        raise AttributeError(f"Property '{key_name}' missing in o {obj}")
    return obj[key_name]
