...
def test_HttpError():
    ...
    assert str(HttpError(msg)) == f"HTTP ERR {msg}"
    print(str(HttpError({"status": 404, "error": "Not found", "message": msg})))
    assert str(HttpError({"status": 404, "error": "Not found", "message": msg})) == (
        "HTTP ERR 404 Not found: An error occurred"
    )
    assert str(
        HttpError(
            err={"statusCode": 404, "error": "Not found", "message": msg},
            req_method="GET",
            base_url="https://bacasable.fenix.rudi-univ-rennes1.fr",
        )
    ) == (
        "HTTP ERR for request 'GET https://bacasable.fenix.rudi-univ-rennes1.fr' -> {'statusCode': 404, 'error': 'Not found', 'message': 'An error occurred'}"
    )
