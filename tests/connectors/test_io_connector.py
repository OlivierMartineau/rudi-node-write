...
file_url = "https://exatow.fenix.rudi-univ-rennes1.fr/storage/download/6d7e6dc4-0aba-4edb-8df2-4eebfec50285"
...
def test_Connector_connection():
    ...
    rudi_connector = Connector(f"{node_url}/catalog/v1")
...
def test_Connector_download():
    ...
    metadata_list = rudi_connector.request(
        "/catalog/v1/resources?available_formats.file_storage_status=available&fields=available_formats"
    )
    ...
    assert rudi_connector.download(f"storage/download/{media_1_id}")

    assert rudi_connector.download("fake.org") is None
    rudi_connector.download("catalog/v1/resources/a")

    with pytest.raises(HttpError):
        rudi_connector.request("catalog/v1/resources/a")
...
def test_https_download():
    ...
    assert https_download(f"{file_url}ba") is None
